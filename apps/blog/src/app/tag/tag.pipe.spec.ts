import { MOCK_POSTS } from 'testing/index';
...
describe('TagPipe', () => {
  ...
  describe("with tag 'only-one-post-has-this'", () => {
    it('should get only one post', () => {
      expect(pipe.transform(MOCK_POSTS, 'only-one-post-has-this')).toEqual([
        MOCK_POSTS[0],
      ]);
    });
  });
  ...
  describe("with tag 'two-posts-share-this'", () => {
    it('should get both two posts', () => {
      expect(pipe.transform(MOCK_POSTS, 'two-posts-share-this')).toEqual(
        MOCK_POSTS,
      );
    });
  });
